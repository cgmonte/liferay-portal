...
test.describe('Manage object relationships through Model Builder', () => {
	...
	test('allows to create relationship with a custom object named Address', async ({
		addNewObjectRelationshipModalPage,
		apiHelpers,
		modelBuilderDiagramPage,
		page,
		...
		const customPostalAddressERC = 'CustomPostalAddress' + getRandomInt();

		const {body: customPostalAddressDefinition} =
			await objectDefinitionAPIClient.postObjectDefinition({
				active: true,
				externalReferenceCode: customPostalAddressERC,
				label: {
		...
		expect(addNewObjectRelationshipModalPage.modalHeader).toBeVisible();
