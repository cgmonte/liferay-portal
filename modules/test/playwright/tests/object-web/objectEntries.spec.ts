...
test.describe('Manage object entries through View Object Entries', () => {
	...
	test('can deselect the last selected option in multiple select picklist and the field is not removed from the DOM when doing so', async ({
		...
		const firsOptionName = objectEntry[multiselectPicklistField.name][0];

		await page.getByTestId(`labelItem-${firsOptionName}`).click();

		await expect
			.soft(page.getByText(firsOptionName, {exact: true}))
			.toBeVisible({timeout: 50});

		const removeOptionButton = page.getByLabel('Remove ' + firsOptionName);
