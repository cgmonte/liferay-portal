	...
	useEffect(() => {
		...
		const timerActions = {
			timerActions: {
				...
				priority: filteredTypeActions.map(({priority}) => priority),
				script: filteredTypeActions.map(({template}) => template),
				sectionsData: filteredTypeActions.map((values) => values),
			},
		};

		updateSelectedItem(timerActions);
